 ...
 *
 * All new Volvo Developer Portal applications enforce PKCE. Generate a [Pkce] instance
 * for each authorization request and include the parameters in the authorization and
 * token exchange steps.
 *
 * **Step 1 — Authorization request:** Include [codeChallenge] and [codeChallengeMethod]
 * as query parameters when redirecting to the Volvo ID login page.
 *
 ...
 * // POST to token endpoint with: grant_type=authorization_code&code=...&code_verifier=${pkce.codeVerifier}
 * ```
 *
 * @property codeVerifier A cryptographically random string (43-128 characters, URL-safe base64)
 * @property codeChallenge The SHA-256 hash of [codeVerifier], base64url-encoded
 * @property codeChallengeMethod Always `S256` (SHA-256)
 *
 * @see [RFC 7636 — PKCE](https://datatracker.ietf.org/doc/html/rfc7636)
 * @see [Volvo PKCE docs](https://developer.volvocars.com/apis/docs/authorisation/#proof-key-for-code-exchange)
 */
public class Pkce private constructor(
    ...
    public companion object {
         ...
         * @return A new [Pkce] instance with cryptographically random verifier and S256 challenge
         */
        @OptIn(ExperimentalEncodingApi::class)
        public fun generate(random: Random = Random): Pkce {
            val bytes = random.nextBytes(32)
            val verifier = Base64.UrlSafe.encode(bytes).trimEnd('=')
            val challengeBytes = sha256(verifier.encodeToByteArray())
            val challenge = Base64.UrlSafe.encode(challengeBytes).trimEnd('=')
...
/**
 * Platform-agnostic SHA-256 implementation using pure Kotlin.
 */
...
internal fun sha256(input: ByteArray): ByteArray {
    ...
    for (i in 0..7) padded[padded.size - 1 - i] = (bitLen ushr (i * 8)).toByte()
